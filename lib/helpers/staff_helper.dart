...
class AppHelper {
  ...
  static Widget managerAddressCell(Address address, Address? tvAddress, Function selectAddress, {Widget? spo2ValueWidget}) {
    ...
    var baseImageName = 'assets/images/status/dummy.png';
    ...
    var tvAddressStatus = -1;
    if (tvAddress != null) {
      tvAddressStatus = tvAddress.status;
    }
    if ((imageName.isEmpty || imageName == 'assets/images/status/dummy.png' || imageName == 'assets/images/status/addr.png') && tvAddressStatus != -1 && address.liveimage.isNotEmpty) {
      bytes = base64Decode(address.liveimage);
    }

    var imageWidget = bytes.isNotEmpty ? Image.memory(
      bytes,
      fit: BoxFit.cover,
    ...
    return Column(
      ...
      children: [
        Container(
          color: address.userType == 'S' ? Color.fromARGB(255, 80, 140, 80) : Color.fromARGB(255, 80, 80, 80),
          height: 21,
          child: Text(
            address.name,
            maxLines: 1,
