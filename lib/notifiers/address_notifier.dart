...
class AddressStore with ChangeNotifier {
  ...
  List<Address> calledList() {
    List<Address> data = [];
    for (var i = 0; i < addressList.length; i++) {
      if (addressList[i].called != 1) {
        continue;
      }
      if (addressList[i].type == 'manager') {
        continue;
  ...
  setCalled(udid, value) {
    ...
    if (value == 1) {
      addressList[index].calledTime = DateTime.now();
    } else {
      addressList[index].calledTime = null;
    }
    notifyListeners();
  }

  setCalledTime(udid, value) {
    final index = _findAddress(udid);
    if (index < 0) {
      return;
    }

    setCalledTimeManager(udid, value);
    print('set called $udid => $value');
    addressList[index].called = 1;
    addressList[index].calledTime = WidgetUtil.parseDatetime(value, format: 'y-M-d HH:mm:ss');
    notifyListeners();
  }
  ...
  setUnableCommunicate(udid, value) {
    final index = _findAddress(udid);
    if (index < 0) {
      return;
    }
    addressList[index].unableCommunication = value;
    setUnableCommunicateManager(udid, value);
    notifyListeners();
  }
  ...
  setBattery(udid, value) {
    final index = _findAddress(udid);
    if (index < 0) {
      return;
    }

    setBatteryManager(udid, value);
    addressList[index].battery = int.parse(value);
    notifyListeners();
  }
