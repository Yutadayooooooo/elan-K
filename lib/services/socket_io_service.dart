...
class SocketIOService {
  ...
  void delegatorLogin(useType) async {
    var group = AppManager.settings['group'];
    ...
    if (AppManager.isManager) {
      if (codesString != null) {
        ...
        for (var i = 0; i < allCodes.length; i++) {
          if (AppManager.authReceives.containsKey(allCodes[i])) {
            codes.add(allCodes[i] + '_STAFF');
          }
        }
      }

      if (!AppManager.authReceives.containsKey(group.toString().replaceAll('_STAFF', ''))) {
