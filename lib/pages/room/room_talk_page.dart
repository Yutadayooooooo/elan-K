...
class RoomTalkPageState extends State<RoomTalkPage>
    with WidgetsBindingObserver, SocketIOServiceDelegate, PaintControllerDelegate {
  bool _init = true;
  bool _active = true;
  Peer peer = Peer();
  ...
  Future<void> _call() async {
    ...
    _rusuTimer = Timer.periodic(const Duration(milliseconds: AppDefine.absenceSec2), (Timer timer) {
      if (!_active || AppManager.status == AppStatus.Talk) {
        print('[DEBUG PRINT]  timeout timer in talk');
        return;
      }
      _timeoutCall();
    });
  ...
  void _timeoutCall() {
    if (socketservice.isConnect()) {
      socketservice.io.emit("call_timeout", [AppManager.selectUser!.id]);
    }
    audio.stopCall();
    AppManager.allTalking = true;
    _close();
  }
