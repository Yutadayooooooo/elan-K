...
var tcp = {
  ...
  listen: function(server) {
    ...
    io.sockets.on('connection', function(socket) {
      ...
      socket.on('call_timeout', function(data) {
        that.callTimeout(this, data);
      });
      ...
      socket.on('call_cancel_push', function(data) {
        console.log('call_cancel_push');
        console.log(data);
        that.callCancelPush(this, data);
      });
  ...
  messageIncomingCallResponse: function(socket, data) {
    ...
    if (data.callResponse === 'accept') {
      ...
      if (client.callToGroup.length > 0) {
        var toGroup = client.callToGroup.substring(0, 1) == '@' ? client.callToGroup.substring(1) : client.callToGroup;
        app.callCancelPushToGroup(toGroup, client.udid, (client.name ? client.name : ''), data.from);
      }
  ...
  messageAnswerResponse: function(socket, data) {
    ...
    if (client.udid in app.pushTimer) {
      clearTimeout(app.pushTimer[client.udid]);
      delete app.pushTimer[client.udid];
    }
    delete app.called[client.udid];
    socket.broadcast.emit('app', {message: 'change_called', info: {client: client.udid, called: 0}});
    ...
    if (toClient.udid in app.pushTimer) {
      clearTimeout(app.pushTimer[toClient.udid]);
      delete app.pushTimer[toClient.udid];
    }
    delete app.called[toClient.udid];
    socket.broadcast.emit('app', {message: 'change_called', info: {client: toClient.udid, called: 0}});
  ...
  disconnect: function(socket) {
    ...
    if (app.clients[socket.id]) {
      ...
      app.disconnectedTimer[udid] = setTimeout(function() {
        console.log('unable to communicate ' + udid);
        app.broadcast('app', {message: "unable_to_communicate", info: {"udid" : udid}});
      }, 30 * 1000);
    }
  },
  ...
  postTalkLog: function(udid, talkInfo) {
    ...
    var options = {
      url: def.AMI_DOMAIN + "delegator/add_talk_log2",
      method: 'POST',
      headers: {
...
console.log(error, body);
  ...
  login: function(socket, data) {
    ...
    if (client.udid in app.disconnectedTimer) {
      clearTimeout(app.disconnectedTimer[client.udid]);
      delete app.disconnectedTimer[client.udid];
    }
    ...
    for (var id in app.clients) {
      ...
      addrClient = app.clients[id];
      if (addrClient.socket.connected) {
        jsonObj[addrClient.udid] = addrClient.toStatusDic();
      } else {
        jsonObj[addrClient.udid] = {"status": def.ECLIENT_STATUS.CLIENT_STATUS_NOT_CONNECT.toString(), "udid": addrClient.udid};
        this.doDisconnect(id);
      }
    }

    socket.emit('app', {message: 'from_server', productName: app.productName, sockID: socket.id, 'data': jsonObj});
  ...
  calledCheck: function(socket, data) {
    ...
    console.log('called check ' + to);
    app.groupcast(client.code, 'called_check', {"MYID" : client.udid, "TO": to});
  },
  ...
  callRequest: function(socket, data) {
    ...
    if (to in app.pushTimer) {
      clearTimeout(app.pushTimer[to]);
      delete app.pushTimer[to];
    }
    ...
    if (to.substring(0, 1) == '@') {
      ...
      for (var id in app.clients) {
        ...
        appNum += 1;
        toClient = app.clients[id];
        if (toClient.status == def.ECLIENT_STATUS.CLIENT_STATUS_TALK) {
          talkingNum += 1;
        }
        if (toClient.group == toGroup) {
          client.callTo.push(toClient.udid);
      ...
      if (appNum === talkingNum && appNum > 0) {
        socket.emit("app", {message: 'all_talking'});
      }
      return;
    }
    ...
    if (toClient != null) {
      ...
      if (toClient.callToGroup.length > 0) {
        app.callCancelPushToGroup(toClient.callToGroup, toClient.udid, (toClient.name ? toClient.name : ''), client.udid);
      }
    }
  ...
  callsRequest: function(socket, data) {
    var client = app.clients[socket.id];
    if (typeof client === 'undefined') {
      return;
    }
    ...
    var to = data.callID;
    var staffs = parseInt(data.staffs);
    console.log('call request ' + client.udid + ' -> ' + to);
    delete app.called[client.udid];
    socket.broadcast.emit('app', {message: 'change_called', info: {client: client.udid, called: 0}});
    ...
    if (to.substring(0, 1) == '@') {
      ...
      for (var id in app.clients) {
        ...
        if (toClient.group == toGroup) {
          client.callTo.push(toClient.udid);
        } else if (toClient.codes.indexOf(toGroup) !== -1) {
          client.callTo.push(toClient.udid);
        }
      }
      if (staffs === talkingNum && staffs > 0) {
        client.callTo = [];
        socket.emit("app", {message: 'all_talking'});
        app.groupcast(toGroup, "app", {message: "call_talking", info: {udid: client.udid, sockID: client.socket.id}});
        return;
      }
      app.groupcast(toGroup, "app", {message: "call", info: {udid: client.udid, sockID: client.socket.id}});
      client.callToGroup = toGroup;
  ...
  callTimeout: function(socket, data) {
    var client = app.clients[socket.id];
    if (typeof client === 'undefined') {
      return;
    }
    ...
    if (to.substring(0, 1) == '@') {
      ...
      function pushOnTimeout() {
        console.log('push on timeout ' + toGroup + ' ' + client.udid);
        app.groupcast(toGroup, "app", {message: "push_notification"});
        app.callPushToGroup(toGroup, client.udid, (client.name ? client.name : ''));
        app.pushTimer[client.udid] = setTimeout(pushOnTimeout, 70 * 1000);
      }
      app.called[client.udid] = utils.dateFormat(new Date(), 'yyyy-mm-dd H:i:s');
      app.pushTimer[client.udid] = setTimeout(pushOnTimeout, 10 * 1000);
    }
  },
  ...
  callCancelPush: function(socket, data) {
    var client = app.clients[socket.id];
    if (typeof client === 'undefined') {
      return;
    }
    ...
    if (callID.substring(0, 1) == '@') {
      var toGroup = callID.substring(1);
      ...
      app.callCancelPushToGroup(toGroup, client.udid, (client.name ? client.name : ''), data.to);
    } else {
      app.callCancelPushToUdid(callID, client.udid, (client.name ? client.name : ''));
    }
    // client.callToGroup = "";
    // client.callTo = [];
  },
  ...
  callAccept: function(socket, data) {
    ...
    if (toClient == null) {
      ...
      if (toClient.status == def.ECLIENT_STATUS.CLIENT_STATUS_CALL) {
        if (!toClient.isCallTo(client.udid) && (toClient.callToGroup !== client.group) && (client.codes.indexOf(toClient.callToGroup) === -1)) {
          isEnd = true;
        } else {
          console.log('call accept ' +  client.udid + ' -> ' + toClient.udid + ':isEnd', isEnd);
        }
      ...
      if (isEnd) {
        console.log('call accept not status_call ' +  client.udid, client.group, client.codes, toClient.callTo, toClient.callToGroup, toClient.status);
      }
    }
  ...
  isCalling: function(socket, data) {
    var client = app.clients[socket.id];
    if (typeof client === 'undefined') {
      return;
    }
    ...
    if (toClient && toClient.status == def.ECLIENT_STATUS.CLIENT_STATUS_CALL) {
      if (app.groupCalling[to] == client.group || client.codes.indexOf(app.groupCalling[to]) !== -1) {
        isCalling = true;
      } else if (client.callTo.indexOf(to)) {
        isCalling = true;
      }
    }

    console.log('is calling ? ' + client.udid + ' -> ' + to + ' : ' + isCalling);
    client.socket.emit("app", {message: "calling", 'calling': isCalling ? '1' : '0'});
  },
  ...
  talk: function(socket, data) {
    ...
    console.log('talk ' + client.udid + ' -> ');
    console.log(client.talkTo);
